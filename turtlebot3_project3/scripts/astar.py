...
def calc_vel(path):
    ...
    for i in range(len(path) - 1):
        ...
        displacement = (np.sqrt((next_node[0] - current_node[0])** 2 + (next_node[1] - current_node[1])** 2))/100

        angle = (next_node[2] - current_node[2])* np.pi *(1)/ 180

        linear_velocity = min(displacement, vel_thresh)  

        angular_velocity = np.clip(angle,-angle_thresh,angle_thresh)

        velocity_list.append((linear_velocity,angular_velocity))
...
class Publish_Vel(Node):
    ...
    def __init__(self,velocity_list):
        ...
        self.timer = self.create_timer(1.5,self.pub_velocity)
...
def ros(args = None):
    ...
    node = Publish_Vel(velocity_list)
    # node.init_velocity(vel_path)
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()     
...
if __name__ == "__main__":
    ...
    start_position = (500 // scaling_factor, 1000// scaling_factor, 0)
    goal_position = (5750// scaling_factor, 1000// scaling_factor,0)
